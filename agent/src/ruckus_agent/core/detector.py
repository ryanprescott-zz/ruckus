...
from .models import (
    ModelInfo  # Still needed for model detection (legacy format)
)
from ruckus_common.models import (
    ...
    MetricType, MetricCollectionMethod,
    # Detection Result Models
    SystemDetectionResult, CPUDetectionResult, GPUDetectionResult,
    FrameworkDetectionResult, FrameworkCapabilities, HookDetectionResult,
    MetricDetectionResult, AgentCapabilityDetectionResult
)
...
class AgentDetector:
    ...
    async def detect_all(self) -> AgentCapabilityDetectionResult:
        """Detect all capabilities."""
        logger.info("AgentDetector starting comprehensive capability detection")
        try:
            # Generate a unique agent ID for this detection run
            import uuid
            ...
            # Create the container result
            result = AgentCapabilityDetectionResult(
                agent_id=agent_id,
                system=system,
                cpu=cpu,
                gpus=gpus,
                frameworks=frameworks,
                models=models,  # Keep as dict for now (legacy format)
                hooks=hooks,
                metrics=metrics
            )
    ...
    async def detect_system(self) -> SystemDetectionResult:
        """Detect system information."""
        logger.debug("Detecting system information")
        try:
            import psutil

            # Map platform.system() to our OSType enum
            ...
            logger.debug(f"System detected: {result.os.value} on {result.hostname}")
            return result
        except Exception as e:
            logger.error(f"System detection failed: {e}")
            raise

    async def detect_cpu(self) -> CPUDetectionResult:
        """Detect CPU information."""
        import psutil

        # Map platform.machine() to our CPUArchitecture enum
        ...
        if machine in ["x86_64", "amd64"]:
            ...
            arch = CPUArchitecture.I386
        else:
            arch = CPUArchitecture.UNKNOWN

        return CPUDetectionResult(
    ...
    async def detect_gpus(self) -> List[GPUDetectionResult]:
    ...
    async def _detect_nvidia_gpu_pynvml(self, device_index: int) -> Optional[GPUDetectionResult]:
    ...
    async def _detect_gpus_pytorch(self) -> List[GPUDetectionResult]:
        ...
        try:
            ...
            if torch.cuda.is_available():
                for i in range(torch.cuda.device_count()):
                    ...
                    gpu_info = GPUDetectionResult(
                        ...
                        detection_method=DetectionMethod.PYTORCH_CUDA
                    )
                    gpus.append(gpu_info)
            ...
            if hasattr(torch.backends, 'mps') and torch.backends.mps.is_available():
                ...
                gpu_info = GPUDetectionResult(
                    ...
                    memory_total_mb=0,  # MPS doesn't report memory limits, using 0 instead of -1 for validation
                    memory_available_mb=0,
                    supported_precisions=supported_precisions,
                    tensor_cores=[TensorCoreGeneration.NONE],  # Apple Silicon uses different acceleration
                    detection_method=DetectionMethod.PYTORCH_MPS
                )
                gpus.append(gpu_info)
    ...
    async def _detect_tensor_capabilities_by_name(self, gpu_name: str) -> Dict:
        ...
        if any(arch in gpu_name_lower for arch in ['a100', 'a6000', 'a5000', 'a4000', 'a2000']):
            # Ampere architecture
            return {
                "tensor_cores": [TensorCoreGeneration.THIRD_GEN],
                "supported_precisions": [PrecisionType.FP32, PrecisionType.TF32, PrecisionType.FP16, 
                                       PrecisionType.BF16, PrecisionType.INT8, PrecisionType.FP64]
            }
            ...
            return {
                ...
                "supported_precisions": [PrecisionType.FP32, PrecisionType.TF32, PrecisionType.FP16, 
                                       PrecisionType.BF16, PrecisionType.INT8, PrecisionType.FP8]
            }
            ...
            return {
                "tensor_cores": [TensorCoreGeneration.THIRD_GEN],
                "supported_precisions": [PrecisionType.FP32, PrecisionType.TF32, PrecisionType.FP16, 
                                       PrecisionType.BF16, PrecisionType.INT8]
            }
        elif any(arch in gpu_name_lower for arch in ['rtx 20', '2080', '2070', '2060', 'titan rtx']):
            # Turing architecture
            return {
                "tensor_cores": [TensorCoreGeneration.SECOND_GEN],
            ...
            # Volta architecture
            return {
                "tensor_cores": [TensorCoreGeneration.FIRST_GEN],
                "supported_precisions": [PrecisionType.FP32, PrecisionType.FP16]
            }
        else:
            # Unknown/older GPU
            return {
                "tensor_cores": [TensorCoreGeneration.NONE],
                "supported_precisions": [PrecisionType.FP32]
            }

    def _map_compute_capability_to_tensor_cores(self, major: int, minor: int) -> List[TensorCoreGeneration]:
        """Map CUDA compute capability to tensor core generations."""
        if major >= 9:  # Hopper (H100, etc.)
            return [TensorCoreGeneration.FOURTH_GEN]
        elif major >= 8:  # Ampere
            return [TensorCoreGeneration.THIRD_GEN]
        elif major == 7 and minor >= 5:  # Turing
            return [TensorCoreGeneration.SECOND_GEN]
        elif major == 7 and minor == 0:  # Volta
            return [TensorCoreGeneration.FIRST_GEN]
        else:
            return [TensorCoreGeneration.NONE]  # No tensor cores

    def _get_supported_precisions_from_compute(self, major: int, minor: int) -> List[PrecisionType]:
        """Get supported precisions based on compute capability."""
        base_precisions = [PrecisionType.FP32]
        
        if major >= 9:  # Hopper
            return base_precisions + [PrecisionType.TF32, PrecisionType.FP16, PrecisionType.BF16, 
                                    PrecisionType.INT8, PrecisionType.FP8, PrecisionType.FP64]
        elif major >= 8:  # Ampere  
            return base_precisions + [PrecisionType.TF32, PrecisionType.FP16, PrecisionType.BF16, 
                                    PrecisionType.INT8, PrecisionType.FP64]
        elif major == 7 and minor >= 5:  # Turing
            return base_precisions + [PrecisionType.FP16, PrecisionType.INT8]
        elif major == 7 and minor == 0:  # Volta
            return base_precisions + [PrecisionType.FP16, PrecisionType.FP64]
        else:
            return base_precisions

    async def _test_precision_support_pytorch(self, device) -> List[PrecisionType]:
        """Test which precisions are actually supported by running operations."""
        supported = [PrecisionType.FP32]  # Always supported
        ...
        try:
            ...
            for precision_type, dtype in test_precisions.items():
                try:
                    ...
                    supported.append(precision_type)
    ...
    async def detect_frameworks(self) -> List[FrameworkDetectionResult]:
    ...
    async def detect_hooks(self) -> List[HookDetectionResult]:
        """Detect available system hooks."""
        logger.debug("Detecting system monitoring hooks")
        hooks = []

        tools = [
            ("nvidia-smi", HookType.GPU_MONITOR),
            ("rocm-smi", HookType.GPU_MONITOR),
            ("htop", HookType.CPU_MONITOR),
            ("iotop", HookType.DISK_MONITOR),
        ]
    ...
    async def detect_metrics(self) -> List[MetricDetectionResult]:
        ...
        basic_metrics = [
            MetricDetectionResult(
                ...
                available=True,
                collection_method=MetricCollectionMethod.TIMER,
                requires=[],
                unit="seconds",
            ...
            MetricDetectionResult(
                ...
                type=MetricType.PERFORMANCE,
                available=True,
                collection_method=MetricCollectionMethod.CALCULATION,
                requires=[],
                unit="tokens/second",
        ...
        if gpus:
            gpu_metrics = [
                MetricDetectionResult(
                    ...
                    available=True,
                    collection_method=MetricCollectionMethod.NVIDIA_SMI,
                    requires=["nvidia-smi"],
                    unit="percent",
