"""Tests for AgentManager core functionality."""
...
from ruckus_server.core.agent_manager import (
    AgentManager,
...
class TestAgentManager:
    ...
    def test_initialization(self, agent_manager_settings):
        """Test AgentManager initialization."""
        server = AgentManager(agent_manager_settings)
        
        assert server.settings == agent_manager_settings
        assert server.logger is not None
        assert server.storage is None  # Not started yet

    def test_initialization_with_default_settings(self):
        """Test AgentManager initialization with default settings."""
        server = AgentManager()
    ...
    async def test_start_and_stop(self, agent_manager):
        ...
        assert agent_manager.storage is not None
        
        # Test that we can stop it
        await agent_manager.stop()

    @pytest.mark.asyncio
    async def test_register_agent_success(self, agent_manager, mock_agent_server_responses):
        """Test successful agent registration."""
        agent_url = "http://localhost:8001"
        mock_response = mock_agent_server_responses["info"]
        
        with patch('ruckus_server.core.agent_manager.AgentProtocolUtility') as mock_utility_class:
            ...
            agent_manager.storage.get_registered_agent_info = AsyncMock(return_value=None)
            agent_manager.storage.register_agent = AsyncMock(return_value=True)
            
            result = await agent_manager.register_agent(agent_url)
    ...
    @pytest.mark.asyncio
    async def test_register_agent_already_registered(self, agent_manager, sample_registered_agent_info):
        ...
        with patch('ruckus_server.core.agent_manager.AgentProtocolUtility') as mock_utility_class:
            ...
            agent_manager.storage.get_registered_agent_info = AsyncMock(
    ...
    @pytest.mark.asyncio
    async def test_register_agent_connection_error(self, agent_manager):
    ...
    @pytest.mark.asyncio
    async def test_register_agent_service_unavailable(self, agent_manager):
    ...
    @pytest.mark.asyncio
    async def test_register_agent_storage_failure(self, agent_manager):
        ...
        with patch('ruckus_server.core.agent_manager.AgentProtocolUtility') as mock_utility_class:
            ...
            agent_manager.storage.get_registered_agent_info = AsyncMock(return_value=None)
            agent_manager.storage.register_agent = AsyncMock(return_value=False)
    ...
    @pytest.mark.asyncio
    async def test_unregister_agent_success(self, agent_manager):
        """Test successful agent unregistration."""
        agent_id = "test-agent-123"
        
        # Mock storage methods
        agent_manager.storage.agent_exists = AsyncMock(return_value=True)
        agent_manager.storage.remove_agent = AsyncMock(return_value=True)
    ...
    @pytest.mark.asyncio
    async def test_unregister_agent_not_registered(self, agent_manager):
        """Test unregistering non-existent agent raises exception."""
        agent_id = "non-existent-agent"
        
        agent_manager.storage.agent_exists = AsyncMock(return_value=False)
    ...
    @pytest.mark.asyncio
    async def test_unregister_agent_storage_failure(self, agent_manager):
        """Test agent unregistration with storage failure."""
        agent_id = "test-agent-123"
        
        agent_manager.storage.agent_exists = AsyncMock(return_value=True)
        agent_manager.storage.remove_agent = AsyncMock(return_value=False)
    ...
    @pytest.mark.asyncio
    async def test_list_registered_agent_info(self, agent_manager, sample_registered_agent_info):
        """Test listing registered agent information."""
        expected_agents = [sample_registered_agent_info]
        
        agent_manager.storage.list_registered_agent_info = AsyncMock(return_value=expected_agents)
        
        result = await agent_manager.list_registered_agent_info()
    ...
    @pytest.mark.asyncio
    async def test_list_registered_agent_info_empty(self, agent_manager):
        """Test listing when no agents are registered."""
        agent_manager.storage.list_registered_agent_info = AsyncMock(return_value=[])
    ...
    @pytest.mark.asyncio
    async def test_get_registered_agent_info(self, agent_manager, sample_registered_agent_info):
        ...
        result = await agent_manager.get_registered_agent_info(agent_id)
        
        assert result == sample_registered_agent_info
        assert result.agent_id == agent_id

    @pytest.mark.asyncio
    async def test_get_registered_agent_info_not_found(self, agent_manager):
        ...
        with pytest.raises(AgentNotRegisteredException) as exc_info:
            await agent_manager.get_registered_agent_info(agent_id)
        
        assert exc_info.value.agent_id == agent_id

    @pytest.mark.asyncio
    async def test_list_registered_agent_status_success(self, agent_manager, sample_registered_agent_info):
        ...
        agent_manager.storage.list_registered_agent_info = AsyncMock(
            return_value=[sample_registered_agent_info]
        )
        
        with patch('ruckus_server.core.agent_manager.SimpleHttpClient') as mock_client_class:
            mock_client = AsyncMock()
            mock_client.get_json.return_value = mock_status.model_dump()
            mock_client_class.return_value = mock_client
            
            result = await agent_manager.list_registered_agent_status()
    ...
    @pytest.mark.asyncio
    async def test_list_registered_agent_status_unavailable_agent(self, agent_manager, sample_registered_agent_info):
    ...
    @pytest.mark.asyncio
    async def test_list_registered_agent_status_empty(self, agent_manager):
    ...
    @pytest.mark.asyncio
    async def test_list_registered_agent_status_concurrent_calls(self, agent_manager, registered_agent_info_factory):
        ...
        agent_manager.storage.list_registered_agent_info = AsyncMock(return_value=agents)
    ...
    @pytest.mark.asyncio
    async def test_get_registered_agent_status_success(self, agent_manager, sample_registered_agent_info):
        ...
        with patch('ruckus_server.core.agent_manager.SimpleHttpClient') as mock_client_class:
            ...
            result = await agent_manager.get_registered_agent_status(sample_registered_agent_info.agent_id)
    ...
    @pytest.mark.asyncio
    async def test_get_registered_agent_status_not_found(self, agent_manager):
        ...
        with pytest.raises(AgentNotRegisteredException) as exc_info:
            await agent_manager.get_registered_agent_status(agent_id)
        
        assert exc_info.value.agent_id == agent_id

    @pytest.mark.asyncio
    async def test_get_registered_agent_status_unavailable(self, agent_manager, sample_registered_agent_info):
    ...
    @pytest.mark.asyncio
    async def test_health_check_healthy(self, agent_manager, sample_registered_agent_info):
        """Test health check when system is healthy."""
        agent_manager.storage.health_check = AsyncMock(return_value=True)
        agent_manager.storage.list_registered_agent_info = AsyncMock(
            return_value=[sample_registered_agent_info]
        )
    ...
    @pytest.mark.asyncio
    async def test_health_check_unhealthy_storage(self, agent_manager):
        """Test health check when storage is unhealthy."""
        agent_manager.storage.health_check = AsyncMock(return_value=False)
        agent_manager.storage.list_registered_agent_info = AsyncMock(return_value=[])
    ...
    @pytest.mark.asyncio
    async def test_health_check_no_storage(self, agent_manager_settings):
    ...
    @pytest.mark.asyncio
    async def test_storage_backend_not_initialized_errors(self, agent_manager_settings):
